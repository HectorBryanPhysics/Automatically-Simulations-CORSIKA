...
int main (){
  ...
  // corsikafile file("/remote/andromedaa/ssaffi/CORSIKA/CTAtest/DATcombined");
 corsikafile file(":v"); //

 while (file.findnextrun())
    {
     run theRun=file.getnextrun();
      cout << "NEvent" << "\t" << "Type" << "\t" << "Weight" << "\t" << "X" << "\t" << "Y" << "\t" << "R" << "\t" <<"Px" << "\t" <<"Py" << "\t" <<"Pz" << "\t" << "P" << "\t" << "Energy" << "\t" << "T" << "\t" << "Zstart" <<endl;
	  ...
	  while (theEvent.findnextparticle())
	    {
	     ...
	     cout << theEvent.eventnum << "\t" << theParticle.type << "\t" << theParticle.weight << "\t" << theParticle.x << "\t" << theParticle.y << "\t" << theParticle.r  << "\t" << theParticle.px << "\t" << theParticle.py << "\t" << theParticle.pz << "\t" << theParticle.p << "\t" << theParticle.energy << "\t" << theParticle.t << "\t" << theParticle.zstart << "\n";
	     
	     //e.g. theParticle.type would give particle type. other quantities:
	     ///px,py,pz in GeV/c, p in eV/c ,x,y in cm, t is arrival time
	     ///of particle at ground in ns since 1st interaction. energy is
	     /// KE in eV. muz is production height in cm (muons with add. info)
	     ///hadgen is hadronic generation. obslev is the observation 
	     ///level
	     ///Follolwing is only for muons with additional info:
